...
<link href="/resources/consultation/bootstrap.min.css" rel="stylesheet">
	 ...
	 <!--list CSS-->
	 <link rel="stylesheet" href="/resources/consultation/list.css">
...
<div class=container>
	<h1 class="page-header">문의하기 게시판</h1>
				...
				<table width="100%"
					...
					<thead>
						<tr>
							...
							<th class="left-aligns">작성일</th>
							<th class="left-align">수정일</th>
						</tr>
					</thead>
					<c:forEach items="${list}" var="board">
						<tr>
							<td class="one"><c:out value="${board.conbno}" /></td>
							
							<!-- Page486 views 폴더 내 board/list.jsp 파일의 일부에 댓글의 숫자 표시를 반영해 줍니다. -->
							<td>
							    <a class="move" href='<c:out value="${board.conbno}"/>'>
							        <c:out value="${board.contitle}" />
							    </a>
							</td>
							<td class="writer"><c:out value="${board.conwriter}" /></td>
							<td class="left-aligns"><fmt:formatDate value="${board.condate}" pattern="yyyy-MM-dd" /></td>
                            <td class="left-align"><fmt:formatDate value="${board.conupdate}" pattern="yyyy-MM-dd" /></td>
						</tr>
					</c:forEach>
				</table>
				<!-- table 태그의 끝! -->
...
<div class="row">
	<div class="col-lg-12">
	<form action="/consultation/list" method="get" id="searchForm">
		...
		<button class="btn btn-default">검색 Search</button>		
			...
			<div class="panel-heading">
				...
				<button id="regBtn" type="button" class="btn btn-xs pull-right">문의하러 가기</button>
			</div>
	</div>
</div>				
	...
	$(document).ready(
			function() {
				...
				$("#regBtn").on("click", function() {
					self.location = "/consultation/register";
				});
				...
				$(".move").on("click", function(e) {
					e.preventDefault();
					actionForm.append("<input type='hidden' name='conbno' value='"+$(this).attr("href")+"'>");
