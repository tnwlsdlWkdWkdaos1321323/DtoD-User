...
var consultationreply = (function() {
	...
	function getList(param, callback, error) {
		var conbno = param.conbno;
		var page = param.page || 1;
		$.getJSON("/consultationreplies/pages/" + conbno + "/" + page + ".json" ,
				function(data) {
	...
	function remove(conrno, callback, error) {
		$.ajax({
			...
			type : 'delete',
			url : '/consultationreplies/delete/' + conrno,
	...
	function update(rereply, callback, error) {
		console.log("RNO 댓글 번호 =" + rereply.conrno);
		$.ajax({
			type : 'put',
			url : '/consultationreplies/modify/' + rereply.rerno,
	...
	function get(conrno, callback, error) {
		$.get("/consultationreplies/" + conrno + ".json", function(result) {
			if (callback) {
				callback(result);
			}
