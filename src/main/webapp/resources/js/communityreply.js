...
var communityreply = (function() {
	
	function add(cmreply, callback, error) {
		console.log("add인식");
		
		$.ajax({
			type : 'post',
			url : '/communityreplies/new',
	...
	function getList(param, callback, error) {
		var cmbno = param.cmbno;
		var page = param.page || 1;
		$.getJSON("/communityreplies/pages/" + cmbno + "/" + page + ".json" ,
				function(data) {
					if (callback) { //콜백 처리된 데이터가 들어왔을 때
						callback(data.cmreplyCnt, data.list);
					}
	...
	function remove(cmrno, callback, error) {
		$.ajax({
			...
			type : 'delete',
			url : '/communityreplies/delete/' + cmrno,
	...
	function update(cmreply, callback, error) {
		console.log("RNO 댓글 번호 =" + cmreply.cmrno);
		$.ajax({
			type : 'put',
			url : '/communityreplies/modify/' + cmreply.cmrno,
			data : JSON.stringify(cmreply),
			contentType : "application/json; charset=utf-8",
			success : function(result, status, xhr) {
				if (callback) {
	...
	function get(cmrno, callback, error) {
		$.get("/communityreplies/" + cmrno + ".json", function(result) {
			if (callback) {
				callback(result);
			}
